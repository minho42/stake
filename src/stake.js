...
const getTransactionHistoryAsx = async (token) => {
  if (!token) {
...
const getCashAvailableForWithdrawal = async (token) => {
  if (!token) {
    throw new Error("getCashAvailableFor");
  }
  let config = {
    method: "get",
    url: "https://global-prd-api.hellostake.com/api/users/accounts/cashAvailableForWithdrawal",
    headers: {
      authority: "global-prd-api.hellostake.com",
      pragma: "no-cache",
      "cache-control": "no-cache",
